/**
 * Generated by orval v8.21.0 🍺
 * Do not edit manually.
 * Api
 * UTB Bus Ticketing API
 * OpenAPI spec version: 0.1.0
 */

export * from './authUser';
export * from './authUserRole';
export * from './clerkSellInput';
export * from './company';
export * from './companyInput';
export * from './companyStat';
export * from './dailyStat';
export * from './dashboardStats';
export * from './getAdminCompaniesParams';
export * from './getAdminHotelsParams';
export * from './getAdminRoutesParams';
export * from './getAdminTripsParams';
export * from './getAdminUsersParams';
export * from './getSalesReportParams';
export * from './healthStatus';
export * from './hotel';
export * from './hotelBooking';
export * from './hotelBookingCallbackInput';
export * from './hotelBookingInitiateResponse';
export * from './hotelBookingInput';
export * from './hotelBookingInputPaymentMethod';
export * from './hotelBookingPaymentMethod';
export * from './hotelBookingPaymentStatus';
export * from './hotelInput';
export * from './hotelSearchResult';
export * from './otpRequest';
export * from './otpResponse';
export * from './otpVerify';
export * from './paginatedCompanies';
export * from './paginatedHotels';
export * from './paginatedRoutes';
export * from './paginatedTrips';
export * from './paginatedUsers';
export * from './passenger';
export * from './paymentCallbackInput';
export * from './paymentInput';
export * from './paymentInputPaymentMethod';
export * from './paymentResponse';
export * from './reserveInput';
export * from './route';
export * from './routeInput';
export * from './salesReport';
export * from './salesReportRow';
export * from './searchHotelsParams';
export * from './searchTripsParams';
export * from './seat';
export * from './seatStatus';
export * from './successResponse';
export * from './ticket';
export * from './ticketPaymentMethod';
export * from './ticketPaymentStatus';
export * from './ticketValidation';
export * from './tripDetail';
export * from './tripDetailStatus';
export * from './tripInput';
export * from './tripSummary';
export * from './tripUpdate';
export * from './tripUpdateStatus';
export * from './user';
export * from './userRole';
export * from './userRoleInput';
export * from './userRoleInputRole';
